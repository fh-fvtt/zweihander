...
export default class ZweihanderPC extends ZweihanderBaseActor {
  ...
  prepareDerivedData(actorData) {
    ...
    const damageModifier = Math.max(0, ...(equippedArmor.map(a => a.data.damageThresholdModifier.value)))
    ...
    const normalTrappingsEnc = carriedTrappings
      .filter(t => t.data.data.encumbrance.value !== 0)
      .map(t => t.data.data.encumbrance.value * t.data.data.CONFIGquantity.value)
      .reduce((a, b) => a + b, 0);
